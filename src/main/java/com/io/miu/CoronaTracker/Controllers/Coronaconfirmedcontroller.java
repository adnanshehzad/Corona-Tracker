...
@RestController
@RequestMapping("/api/v1/confirmed")
public class Coronaconfirmedcontroller {
    @Autowired
    CoronaconfirmedServiceImpl coronaVirusService;

    @GetMapping("/")
    public ResponseEntity<?> getAllData() throws IOException, InterruptedException {
        List<CoronaData> resultData=coronaVirusService.fetchcasesdata();
    ...
    public ResponseEntity<?> getByCountry(@PathVariable("country") String country) throws IOException, InterruptedException {
        CoronaData countrydata=coronaVirusService.getcasesByCountry(country);
        if (countrydata!=null){
